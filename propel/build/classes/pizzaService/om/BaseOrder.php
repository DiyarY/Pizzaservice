...
abstract class BaseOrder extends BaseObject implements Persistent
{
     ...
     * @throws PropelException - if unable to parse/validate the date/time value.
     */
    public function getCompletedAt($format = '%X')
    {
    ...
    public function setCustomersId($v)
    {
        ...
        if ($this->customers_id !== $v) {
            $this->customers_id = $v;
            $this->modifiedColumns[] = OrderPeer::CUSTOMERS_ID;
        }
    ...
    public function setCompletedAt($v)
    {
        ...
        if ($this->completed_at !== null || $dt !== null) {
            $currentDateAsString = ($this->completed_at !== null && $tmpDt = new DateTime($this->completed_at)) ? $tmpDt->format('H:i:s') : null;
            $newDateAsString = $dt ? $dt->format('H:i:s') : null;
            if ($currentDateAsString !== $newDateAsString) {
                $this->completed_at = $newDateAsString;
                $this->modifiedColumns[] = OrderPeer::COMPLETED_AT;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->total = ($row[$startcol + 2] !== null) ? (double) $row[$startcol + 2] : null;
            $this->created_at = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
            $this->completed_at = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
            $this->resetModified();
            ...
            return $startcol + 5; // 5 = OrderPeer::NUM_HYDRATE_COLUMNS.
    ...
    public function ensureConsistency()
    {
        ...
        if ($this->aCustomer !== null && $this->customers_id !== $this->aCustomer->getId()) {
            $this->aCustomer = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(OrderPeer::CUSTOMERS_ID)) {
            $modifiedColumns[':p' . $index++]  = '`customers_id`';
        }
        if ($this->isColumnModified(OrderPeer::TOTAL)) {
            $modifiedColumns[':p' . $index++]  = '`total`';
        }
        if ($this->isColumnModified(OrderPeer::CREATED_AT)) {
            $modifiedColumns[':p' . $index++]  = '`created_at`';
        }
        if ($this->isColumnModified(OrderPeer::COMPLETED_AT)) {
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[4], $arr)) $this->setCompletedAt($arr[$keys[4]]);
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(OrderPeer::CUSTOMERS_ID)) $criteria->add(OrderPeer::CUSTOMERS_ID, $this->customers_id);
        ...
        if ($this->isColumnModified(OrderPeer::COMPLETED_AT)) $criteria->add(OrderPeer::COMPLETED_AT, $this->completed_at);
    ...
    {
        if ($this->aCustomer === null && ($this->customers_id !== null) && $doQuery) {
            $this->aCustomer = CustomerQuery::create()->findPk($this->customers_id, $con);
    ...
    public function setOrder_Pizzas(PropelCollection $order_Pizzas, PropelPDO $con = null)
    {
        ...
        foreach ($order_PizzasToDelete as $order_PizzaRemoved) {
            $order_PizzaRemoved->setOrder(null);
        }
     ...
     * @return PropelObjectCollection|Order_Pizza[] List of Order_Pizza objects
     */
    public function getOrder_PizzasJoinPizza($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = Order_PizzaQuery::create(null, $criteria);
     ...
     *
     * @param  Pizza $pizza The Order_Pizza object to relate
     * @return Order The current object (for fluent API support)
     */
    ...
    protected function doAddPizza(Pizza $pizza)
    {
        ...
        if (!$pizza->getOrders()->contains($this)) { $order_Pizza = new Order_Pizza();
            $order_Pizza->setPizza($pizza);
            $this->addOrder_Pizza($order_Pizza);

            $foreignCollection = $pizza->getOrders();
